# Starter(启动器)
starter 是对以前spring整合所需求得jar包的封装，直接调用starter可以很方便的组合想要的组件，如：mybaits、jpa、mq、shiro 等

pom.xml中：都应改实现springboot `parent`
```
<parent>
...
```
>他使导入其他spring 相关jar不用考虑版本，它会自己检索合适的版本。

## 常用的starter（https://github.com/ityouknow/awesome-spring-boot/blob/master/starter-list.md）
...
>@ConfigurationProperties 是springboot新加入的注解，主要用于配置文件中的指定键值对映射到一个java实体类上。

>@EnableConfigurationProperties：自由选择被@ConfigurationProperties标注的类
```
